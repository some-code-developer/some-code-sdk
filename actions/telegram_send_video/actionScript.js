...
const apiUrl = `https://api.telegram.org/bot${actionParameters.connection.password}/sendVideo`;
...
try {
  ...
  if (response.status !== 200) throw new Error(`Failed to send video to Telegram: ${response.status} ${response.statusText}`);
} catch (e) {
